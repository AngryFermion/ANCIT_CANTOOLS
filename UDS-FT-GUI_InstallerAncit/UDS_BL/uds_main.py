...
import OS.threadMaster as thread
...
def AncitTransferExit(client):
    ...
    try:
        ...
        screen_log.log_message(log_area=logg,message="Transfer Exit complete")
    except NegativeResponseException as e:
...
def AncitEcuReset(client):
    ...
    screen_log.log_message(log_area=logg,message="ECU Reset Complete")
    thread.stopTask()
...
def AncitTransferData(client,filepath,start_addr,end_addr):
    ...
    screen_log.log_message(log_area=logg,message="Dowloading hex file into target flash...")
    try:
        total_block = 1
        block = 1
        address = start_addr
        ...
        while(transfer_data == 1):
            ...
            progress_message = ""
            progress_message = (str)(round(progress,1)) + "%" + "Flashing Complete"
            screen_log.log_message(log_area=logg,message=progress_message)
        ...
        if service == 0:
            with Client(can_inst,  request_timeout=10,config=config) as client:
                ...
                try:
                    ...
                    screen_log.log_message(log_area=logg,message="Erase Routine complete.\n")
