...
def show_main_window(img_path,img_ico):
    ...
    global root
    root = tk.Tk()
    root.title("ANCIT CAN FLASH TOOL")
    root.iconbitmap(img_ico)
    ...
    top_hexLabel_frame = tk.Frame(root)
    top_hexLabel_frame.pack(pady=5, padx=5, fill=tk.X)
    ...
    label_hexPath = tk.Label(top_hexLabel_frame, text="Insert Hex file path:")
    label_hexPath.pack(side=tk.LEFT,pady=2)  # Add some vertical padding
    ...
    # Frame for log area label - Status
    top_Status_frame = tk.Frame(root)
    top_Status_frame.pack(pady=5, padx=5, fill=tk.X)

    label_Status = tk.Label(top_Status_frame, text="Flashing Status:")
    label_Status.pack(side=tk.LEFT,pady=2)  # Add some vertical padding
...
# Create a label to hold the image
    ...
    label.pack(side=tk.RIGHT, padx=2)
